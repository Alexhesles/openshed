...
import { Wrench, Hammer, Leaf, Zap, Droplets, Clock, CheckCircle, XCircle } from 'lucide-react'
...
const STATUS = {
  ...
  approved:  { label:'Approved! ✓',       color:'#34C759', bg:'#E8F9ED', Icon:CheckCircle  },
...
export default function MyLoansScreen() {
  ...
  const fetchLoans = async () => {
    ...
    const { data } = await supabase
      .from('loans')
      .select('*, tools(name, brand, category, health, is_free, price_per_day), profiles!lender_id(full_name)')
        ...
        {loading ? (
          ...
          return (
            <div key={loan.id} style={{ background:C.card, borderRadius:16, marginBottom:12, overflow:'hidden', boxShadow:C.sh, border:`1px solid ${C.brd}` }}>
              ...
              {/* Status bar */}
              <div style={{ margin:'0 14px 14px', background:st.bg, borderRadius:10, padding:'10px 12px', display:'flex', alignItems:'center', gap:8, border:`1px solid ${st.color}33` }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:C.t2, marginTop:1 }}>
                    Requested {new Date(loan.created_at).toLocaleDateString('en-US',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}
                  </div>
                </div>
              </div>
            </div>
