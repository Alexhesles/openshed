...
export default function EditToolScreen({ tool, onBack, onSaved }) {
  const [name,     setName]     = useState(tool.name || '')
  const [brand,    setBrand]    = useState(tool.brand || '')
  const [category, setCategory] = useState(tool.category || 'Power Tools')
  const [isFree,   setIsFree]   = useState(tool.is_free ?? true)
  const [price,    setPrice]    = useState(tool.price_per_day || '')
  const [health,   setHealth]   = useState(tool.health || 80)
  const [photo,    setPhoto]    = useState(null)
  const [preview,  setPreview]  = useState(tool.photo_urls?.[0] || null)
  ...
  const handleSave = async () => {
    ...
    let photoUrls = tool.photo_urls
    if (photo) {
      const ext = photo.name.split('.').pop()
      const filePath = `${user.id}/${Date.now()}.${ext}`
      const { error: uploadError } = await supabase.storage.from('tool-photos').upload(filePath, photo)
      if (!uploadError) {
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:C.bg }}>
      <div style={{ background:C.card, padding:'8px 14px 14px', borderBottom:`1px solid ${C.brd}`, flexShrink:0 }}>
        ...
        <div style={{ fontWeight:800, fontSize:20, color:C.t1 }}>Edit Tool</div>
        <div style={{ fontSize:13, color:C.t2, marginTop:2 }}>Update your listing</div>
      </div>
      <div style={{ flex:1, overflowY:'auto', padding:16 }}>
        {success ? (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', paddingTop:60 }}>
            ...
            <div style={{ fontWeight:800, fontSize:20, color:C.t1 }}>Tool Updated!</div>
          </div>
        ) : (
          <>
              ...
              <label style={{ display:'block', cursor:'pointer' }}>
                ...
                <div style={{ height:160, background:preview?'transparent':C.cardAlt, borderRadius:16, border:`2px dashed ${preview?C.blue:C.brdM}`, overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:8 }}>
                  {preview
                    ...
                    : <><Camera size={28} color={C.t3} strokeWidth={1.5}/><span style={{ fontSize:13, color:C.t2 }}>Tap to change photo</span></>
                  }
                </div>
              </label>
            </div>
