...
import { Search, Truck, Hammer, Wrench, Leaf, Droplets, ChevronRight, MapPin } from 'lucide-react'
...
export default function BrowseScreen({ goRealTool }) {
  ...
  useEffect(() => { fetchTools(); askLocation() }, [])

  const askLocation = () => {
    if (!navigator.geolocation || locAsked) return
    setLocAsked(true)
  ...
  const fetchTools = async () => {
    ...
    const { data } = await supabase
      .from('tools')
      .select('*, profiles(id, full_name, trust_score, latitude, longitude)')
  ...
  const withDist = tools.map(t => {
    let dist = null
    if (userLoc && t.profiles?.latitude && t.profiles?.longitude) {
      dist = haversine(userLoc.latitude, userLoc.longitude, t.profiles.latitude, t.profiles.longitude)
    }
    return { ...t, dist }
  })

  const filtered = withDist.filter(t => {
    const mq = t.name.toLowerCase().includes(query.toLowerCase()) || (t.brand||'').toLowerCase().includes(query.toLowerCase())
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflowY:'auto', background:C.bg }}>
      <div style={{ background:C.card, padding:'8px 14px 12px', borderBottom:`1px solid ${C.brd}`, flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
          <div style={{ fontSize:22, fontWeight:800, color:C.t1, letterSpacing:'-0.4px' }}>Browse Tools</div>
          {userLoc && (
            <div style={{ display:'flex', alignItems:'center', gap:4, background:C.greenL, borderRadius:20, padding:'4px 10px', border:`1px solid ${C.green}33` }}>
              <MapPin size={11} color={C.green} strokeWidth={2}/>
              <span style={{ fontSize:11, fontWeight:600, color:C.green }}>Location on</span>
            </div>
          )}
        </div>
        ...
        <div style={{ display:'flex', gap:8, marginTop:10, overflowX:'auto', paddingBottom:2 }}>
          {CATEGORIES.map((c, i) => <Chip key={i} active={category===c} onPress={() => setCategory(c)}>{c}</Chip>)}
        </div>
      </div>
                ...
                return (
                  <div key={t.id} onClick={() => goRealTool(t)} className="tp"
                    ...
                    <div style={{ padding:'10px 12px 12px' }}>
                      ...
                      <div style={{ fontSize:11, color:C.t2, marginTop:1 }}>by {t.profiles?.full_name || 'Neighbor'}</div>
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:6 }}>
                        <div style={{ fontWeight:700, fontSize:14, color:t.is_free?C.green:C.t1 }}>
                          {t.is_free ? 'Free' : `$${t.price_per_day}/day`}
                        </div>
                        {t.dist !== null && (
                          <div style={{ display:'flex', alignItems:'center', gap:3 }}>
                            <MapPin size={10} color={C.t3} strokeWidth={1.5}/>
                            <span style={{ fontSize:10, color:C.t3 }}>{t.dist < 0.1 ? '<0.1' : t.dist.toFixed(1)} mi</span>
                          </div>
                        )}
                      </div>
                    </div>
